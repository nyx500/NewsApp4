...
# LIME explanation library: class for text-based explanations
from lime.lime_text import LimeTextExplainer
# Class for generating extra feature score based explanations
...
def explainPredictionWithLIME(
    ...
    # Create a LIME tabular explainer for the engineered features
    """"
        Explains predictions on tabular (i.e. matrix) data. For numerical features, 
        perturb them by sampling from a Normal(0,1) and doing the inverse operation of mean-centering and scaling, 
        according to the means and stds in the training data (LIME TabularExplainer docs)
    """
    tabular_explainer = LimeTabularExplainer(
        # Uses the extra features array (but would ideally use training data distrubtions)
        training_data=extra_features_array,
        feature_names=extra_feature_names,
        class_names=["real", "fake"],
        discretize_continuous=False,
        mode="regression",  # Uses probabilities rather than binary labels 
        sample_around_instance=True # "Will sample continuous features in perturbed samples from a normal centered at the instance being explained.""
    )
    ...
    # Generates the LIME tabular explanations for tabular features
...
def displayAnalysisResults(explanation_dict, container, news_text, feature_extractor, FEATURE_EXPLANATIONS):
    ...
    with col2:
        ...
        # Creates a combined bar chart for comparing word and extra features
        ...
        # Get top 10 BOTH real & fake word features by absolute importance score
        ...
        # Add abs importance column to sort again
